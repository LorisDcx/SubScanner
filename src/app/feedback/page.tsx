...
export default function FeedbackPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Afficher un message différent si c'est en mode dev
      if (data.devMode) {
        console.log("📝 Message envoyé en mode développement (non envoyé par email)");
      }
            ...
            {status === "error" && (
              <div className="rounded-xl bg-rose-500/10 border border-rose-500/20 p-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-rose-300/80 text-sm mt-1">{errorMessage}</p>
                </div>
              </div>
            )}
